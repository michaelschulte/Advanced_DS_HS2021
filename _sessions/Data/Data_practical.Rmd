...
<p align="center" width="100%">
  <img src="https://www.institute4learning.com/blog/wp-content/uploads/2020/02/data.jpg" alt="Data" style="width:100%;height:280px">
  <br>
  <font style="font-size:10px">from <a href="www.institute4learning.com">https://www.institute4learning.com</a></font>
</p>
...
```{r, out.width = "8%", fig.align = 'left', eval = TRUE, echo = FALSE}
knitr::include_graphics("image/SPSS.jpg")
```
...
1. Verwende `read_html()` um den Satz auf dieser Webseite: https://dataquestio.github.io/web-scraping-pages/simple.html in das Obekt `einfach` einzulesen
...
```{r}
einfach <- read_html("https://dataquestio.github.io/web-scraping-pages/simple.html")
```

2. Printe `einfach` und finde heraus welches Tag für uns interessant ist, um den Satz zu extrahieren.

3. Genau! Wir sind am Tag `<p>` interessiert - unter Verwendung von `html_nodes()` und dann `html_text()` extrahiere den Satz: Here is some simple content for this page.
...
4. Nun wollen wir eine ganze Tabelle extrahieren. Gehe dafür auf die Seite: <a href = "https://ourworldindata.org/famines">https://ourworldindata.org/famines</a>, auf der Daten über Hungersnöte abgebildet sind.

5. Am Ende der Seite (langer scroll!) findest du eine Tabelle, in der Hungersnöte von 1846 - 2011 gelistet sind. Diese Tabelle werdden wir einlesen.

6. Finde zuerst die ID der Tabelle heraus (siehe Trick in Chrome, oder rechte Maustaste und `Inspect`). Du suchst `id = tablepress-XX`. Speichere diese ID unter `css_selector` und den url im Object `url` ab.
...
7. Lies nun die Webseite mit `read_html()` ein und schreib das Ergebnis in das Objekt `famines`.
...
```{r}
famines <- read_html(url)
```

8. Wähle nun mittels `html_element()` den css selector der Tabelle aus, den du weiter oben gespeichert hast.
...
9. Fast fertig! Die Darstellung ist noch nicht sehr leserlich - verwende `html_table()`, um das html in einen R dataframe umzuwandeln.
